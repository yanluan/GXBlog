...
class Model {
	 ...
	 * 非过滤字段列表.
	 * 
	 * 被 沈阳晨信网络科技有限公司 增加，扩展“设定费过滤字段功能”功能。
	 * 
	 */
	protected $unfilterFields    =   array();
	...
	protected function _read_data($data) {
		...
		if (! empty ( $this->_map ) && C ( 'READ_DATA_MAP' )) {
			Log::record("Table field map: " . var_export($this->_map,true), Log::INFO);
			Log::record("beford:" . var_export($data,true),Log::INFO);
			foreach ( $this->_map as $key => $val ) {
				/* 被沈阳晨信网络科技修改  修复$this->_map中key和value相同时，处理数据的bug */
				if($key === $val) continue;
			...
			Log::record("after:" . var_export($data,true),Log::INFO);
		}
		return $data;
	}
	...
	public function create($data = '', $type = '') {
		...
		if (empty ( $data )) {
			$data = I ( 'post.' );
			// =========================================
			// 忽略无需过滤的参数，沈阳晨信网络科技有限公司 修改。
			foreach ($this->unfilterFields as $v){
