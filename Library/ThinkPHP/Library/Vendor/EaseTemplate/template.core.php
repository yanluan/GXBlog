...
class ETCore {
	 ...
	 * 声明模板用法
	 */
	function ETCoreStart($set = array(
				'ID'		 =>'1',					//缓存ID
				'TplType'	 =>'htm',				//模板格式
				'CacheDir'	 =>'cache',				//缓存目录
	 ...
	 * set_file(文件名,设置目录);
	 */
	function set_file($FileName, $NewDir = '') {
		// 当前模板名
...
<!-- END -->";
	 ...
	 * 编译解析处理
	 */
	function CompilePHP($content = '', $cachename = '') {
		if ($content) {
			// 如果没有安全文件则自动创建
	 ...
	 * 静态文件写入
	 */
	function htm_w($w_dir = '', $w_filename = '', $w_content = '') {
		$dvs = '';
	 ...
	 */
	function writer($filename, $data = '', $mode = 'w') {
		if (trim ( $filename )) {
